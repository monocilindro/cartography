...
`propSymbolsTypoLayer()` creates a map of symbols that are proportional to values of a first variable and colored to reflect the modalities of a second qualitative variable. A combination of `propSymbolsLayer()` and `typoLayer()` arguments is used. 
